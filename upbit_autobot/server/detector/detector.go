...
var TheIchimokuCycleOngoing = false
...
func StartDetectorCycle(client *upbit.Upbit) {
	go StartBuyWithIchimokuDetectorBot(client)
...
// important : amount changed from coin amount to krw
// TODO
func StartBuyWithIchimokuDetectorBot(client *upbit.Upbit) {
	singleton.InstanceLogger().Msgs <- "구매 감시 봇(일목) 작동 시작."
	for {
		// 전체 사이클 회수(최대 10개니 대략 5~6초 소요)
		// whole cycle wait(without it, maximum it takes 5-6seconds)
		TheIchimokuCycleOngoing = false
		time.Sleep(time.Millisecond * 5000)
		TheIchimokuCycleOngoing = true

		if len(singleton.InstanceBuyTargetIchimokuItems().Items) == 0 {
			continue
		}
		...
		for i := 0; i < len(singleton.InstanceBuyTargetIchimokuItems().Items); i++ {
			item := singleton.InstanceBuyTargetIchimokuItems().Items[i]
			...
			if len(marketMap) != 0 {
				if _, ok := marketMap[item.CoinMarketName]; !ok {
					...
					singleton.InstanceBuyTargetIchimokuItems().Items = append(singleton.InstanceBuyTargetIchimokuItems().Items[:i], singleton.InstanceBuyTargetIchimokuItems().Items[i+1:]...)
					singleton.SaveStrategyBuyTargetIchimokuItems()
			...
			candles, err := order.GetNaiveCandles(client, item.CoinMarketName, item.ConversionLine, minutes)
			if err != nil {
				singleton.InstanceLogger().Errs <- err
				continue
			}
			...
			line := indicator.GetIchimokuConversionLine(candles)
			...
			sendAllBalanceAndIchimokuItem(client, item)
			...
			data := model.ItemCollectionForSocketNew()
			data.BuyStartegyIchimokuItem = item
			singleton.InstanceItemsCollectionCh() <- data
			...
			if err != nil {
				singleton.InstanceLogger().Errs <- fmt.Errorf("구매 볼륨 정보 취득 오류로 감시봇 진입 실패했습니다. #에러 발생 코인 이름: %s", item.CoinMarketName)
				continue
			}
		...
		for i := 0; i < len(singleton.InstanceSellTargetItems().BoughtItems); i++ {
			...
			if sellTargetItem.ProfitTargetPrice != 0. {
				...
				if price > sellTargetItem.ProfitTargetPrice {
					...
					// buy strategy item loop(ichimoku)
					for j := 0; j < len(singleton.InstanceBuyTargetIchimokuItems().Items); j++ {
						if singleton.InstanceBuyTargetIchimokuItems().Items[j] == nil {
							continue
						}

						// delete if count == 0
						if singleton.InstanceBuyTargetIchimokuItems().Items[j].ItemId == sellTargetItem.ItemId {
							if singleton.InstanceBuyTargetIchimokuItems().Items[j].PurchaseCount == 0 {
								singleton.InstanceBuyTargetIchimokuItems().Items = append(singleton.InstanceBuyTargetIchimokuItems().Items[:j], singleton.InstanceBuyTargetIchimokuItems().Items[j+1:]...)
								singleton.SaveStrategyBuyTargetIchimokuItems()
...
func sendAllBalanceAndIchimokuItem(client *upbit.Upbit, item *model.BuyStrategyIchimokuItemInfo) {
	singleton.RefreshAccount(client)
	coinbalances := model.CoinBalances{Balances: []*model.CoinBalance{}}
	for _, balance := range singleton.InstanceCoinBalances() {
		isSmall := checkSmallbalance(balance.AvgBuyPrice, balance.Balance)
